    ...
    public var body: some ReducerOf<Self> {
        ...
        Reduce { (state: inout State, action: Action) -> Effect<Action> in
            ...
            case .didTapSearchHereButton:
                ...
                return .merge(
                    .run { _ in await analytics.logEvent(event: event) },
            ...
            case let .favoritePhotoBoothsResponse(.success(photoBooths), shouldLogViewEvent, generation):
                ...
                let viewEventEffect: Effect<Action> = shouldLogViewEvent
                    ? .run { _ in await analytics.logEvent(event: MapAnalyticsEvent.favoriteBoothView(favoriteBoothCount: favoriteBoothCount)) }
                    : .none
                return .merge(
            ...
            case let .didSelectDirectionApp(appType):
                ...
                return .merge(
                    .run { _ in await analytics.logEvent(event: MapAnalyticsEvent.mapRouteClick(mapType: appType)) },
